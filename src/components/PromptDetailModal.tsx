...
const PromptDetailModal: React.FC<PromptDetailModalProps> = ({
  ...
  const [currentPromptId, setCurrentPromptId] = useState<string | number | undefined>(prompt?.id);

  const animationTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const messageTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  const hasChanges = prompt && (editedTitle !== prompt.title || editedText !== prompt.text);
  ...
  return (
    <div className={`detail-modal-overlay ${animationClass}`}>
      <div className={`detail-modal-content ${animationClass}`}>
        ...
        <div className={`detail-main-content-area ${isEditing ? 'is-editing' : ''}`}>
          {isEditing ? (
            <>
              <input
                type="text"
          ...
          <div className={`detail-empty-warning-message ${showEmptyWarning ? 'show-warning' : ''}`}>
            Title and prompt cannot be empty.
          </div>
          <div className={`detail-success-message ${showSuccessMessage ? 'show-success' : ''}`}>
